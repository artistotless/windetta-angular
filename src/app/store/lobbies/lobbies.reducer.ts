...
const _initialState: ILobbiesState = adapter.getInitialState({
    currentLobby: { LobbyId: "", RoomIndex: -1, UserId: "" },
    isLoading: false,
    error: null,
    isCached: false
})
...
export const lobbyReducers = createReducer(_initialState,
    ...
    on(LobbyActions.getCurrent, (state) => state),
    on(LobbyActions.getCurrentSuccess, (state, currentLobby) => ({ ...state, currentLobby: currentLobby })),
    on(LobbyActions.getCurrentFailure, (state) => ({ ...state, currentLobby: { LobbyId: "", RoomIndex: -1, UserId: "" } })),
